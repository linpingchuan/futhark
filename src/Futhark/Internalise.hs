...
internaliseValBind (E.ValBind entry name _ t e loc doc) =
  internaliseFunBind $ E.FunBind entry name Nothing t [] [] e loc doc
...
internaliseFunBind fb@(E.FunBind entry ofname _ (Info rettype) tparams params body loc _) = do
...
generateEntryPoint (E.FunBind _ ofname _ (Info rettype) _ orig_params _ loc _) =
...
internaliseExp desc (E.LetFun ofname (tparams, params, retdecl, Info rettype, body) letbody loc) = do
  internaliseFunBind $ E.FunBind False ofname retdecl (Info rettype) tparams params body loc Nothing
